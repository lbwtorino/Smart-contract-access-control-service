...
CStorage = new LocalStorage('./data/position/MultipleC');
...
function grantTickets(type, message, verify_private_key, access_time, contract_name, deployed_address) {
    if (CStorage.getItem("index") == null){
        CStorage.setItem("index", 0);
    }
    switch(type) {
        case 1:
            ...
            var _index = 0;
            if (message[1] == true){
                _index = parseInt(CStorage.getItem("index"));
                _index += 1;
                CStorage.setItem("index", _index);  
            }
...
function verifyPolicy(type, msg, contract_name){
    var counter = 0;
    if (counter == 0){
        white_list = JSON.parse(policyStorage.getItem('MultipleC')).white;
        black_list = JSON.parse(policyStorage.getItem('MultipleC')).black;
