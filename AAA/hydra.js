...
function grantTickets(type, message, verify_private_key, access_time, contract_name, deployed_address) {
    ...
    switch(type) {
        ...
        case 2:
            ...
            var _whole_msg = deployed_address.substring(2)+pad(64-deployed_address.substring(2).length) +type.toString() +access_time.toString() + _index.toString() + message[0].substring(2)+pad(64-message[0].substring(2).length) + web3.utils.sha3('setValue(uint256,bytes32[],uint256[],uint256[],uint8[],bytes1[],bytes32[],bytes32[])').substring(0,10);
            console.log(_whole_msg)            
            var _vsignature = web3.eth.accounts.sign(_whole_msg, verify_private_key).v.toString();
            var _rsignature = web3.eth.accounts.sign(_whole_msg, verify_private_key).r.toString();
            var _ssignature = web3.eth.accounts.sign(_whole_msg, verify_private_key).s.toString();
        ...
        case 3:
            ...
            var _whole_msg = receiver +type.toString() +access_time.toString() + _index.toString()+ message[0].substring(2)+pad(64-message[0].substring(2).length) + web3.utils.sha3('setValue(uint256,bytes32[],uint256[],uint256[],uint8[],bytes1[],bytes32[],bytes32[])').substring(0,10)+ argValue;
            console.log(_whole_msg) 
            var _vsignature = web3.eth.accounts.sign(_whole_msg, verify_private_key).v.toString();
            var _rsignature = web3.eth.accounts.sign(_whole_msg, verify_private_key).r.toString();
            var _ssignature = web3.eth.accounts.sign(_whole_msg, verify_private_key).s.toString();
...
function verifyPolicy(type, msg, contract_name){
    var counter = 0;
    if (counter == 0){
        white_list = JSON.parse(policyStorage.getItem('hydraSmacs')).white;
        black_list = JSON.parse(policyStorage.getItem('hydraSmacs')).black;
        fun_white_list = JSON.parse(policyStorage.getItem(contract_name)).function.setValue.white;
        fun_black_list = JSON.parse(policyStorage.getItem(contract_name)).function.setValue.black;
        var_white_list = JSON.parse(policyStorage.getItem(contract_name)).args.value.white;
        var_black_list = JSON.parse(policyStorage.getItem(contract_name)).args.value.black;
        counter = counter+1;
    }
    switch(type){
        case 1:
