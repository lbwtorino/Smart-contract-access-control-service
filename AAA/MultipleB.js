...
BStorage = new LocalStorage('./data/position/MultipleB');
...
function grantTickets(type, message, verify_private_key, access_time, contract_name, deployed_address) {
    if (BStorage.getItem("index") == null){
        BStorage.setItem("index", 0);
    }
    switch(type) {
        case 1:
            ...
            var _index = 0;
            if (message[1] == true){
                _index = parseInt(BStorage.getItem("index"));
                _index += 1;
                BStorage.setItem("index", _index);  
            }
...
function verifyPolicy(type, msg, contract_name){
    var counter = 0;
    if (counter == 0){
        white_list = JSON.parse(policyStorage.getItem('MultipleB')).white;
        black_list = JSON.parse(policyStorage.getItem('MultipleB')).black;
