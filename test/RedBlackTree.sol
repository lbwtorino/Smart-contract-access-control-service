...
library RedBlackTree {
    ...
    function find(Tree storage tree, uint value) public constant returns (uint64 parentId) {
        uint64 id = tree.root;
        parentId = id;
        // uint itemValue = tree.items[id].value;
        while (id != 0) {
            //lbw
    ...
    function find1(Tree storage tree, uint value) public constant returns (bool) {
        // parentId = id;
        // _value = value;
        // itemValue = tree.items[id].value;
        // test = (value < tree.items[id].value);
        if (value == tree.items[tree.root].value){
            return true;
        }
        uint64 id = tree.root;
        while (id != 0){
    ...
    function delete2(Tree storage tree, uint64 n) private {
            ...
            !tree.items[sRight].red) 
        {
            ...
            if (tree.items[p].red && !tree.items[s].red && !tree.items[sLeft].red && !tree.items[sRight].red) 
            {
                ...
                if (!tree.items[s].red) {
                    if (n == tree.items[p].left && !tree.items[sRight].red &&
                        tree.items[sLeft].red) 
                    {
                        tree.items[s].red = true;
                        tree.items[sLeft].red = false;
                        rotateRight(tree, s);
                    } else if (n == tree.items[p].right && !tree.items[sLeft].red &&
                        tree.items[sRight].red) 
                    {
                        tree.items[s].red = true;
