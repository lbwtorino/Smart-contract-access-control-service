...
AStorage = new LocalStorage('./data/position/Banker');


function synchronizeData(verify_private_key, public_key, owner_address, contract_name, deployed_address, white_list, black_list, update_key, tools){
    ...
    if (getCounter(contract_name) == null){
        ...
        AStorage.setItem("index", 0);
    }
    var method_name = "withdraw";
    addPolicy(contract_name, method_name, white_list, black_list, tools);
}
...
function grantTickets(type, message, verify_private_key, access_time, contract_name, deployed_address) {
    if (AStorage.getItem("index") == null){
        AStorage.setItem("index", 0);
    }
    switch(type) {
        case 1:
            ...
            var _index = 0;
            if (message[1] == true){
                _index = parseInt(AStorage.getItem("index"));
                _index += 1;
                AStorage.setItem("index", _index);  
            }
        ...
        case 2:
            ...
            var _whole_msg = deployed_address.substring(2)+pad(64-deployed_address.substring(2).length) +type.toString() +access_time.toString() + _index.toString() + message[0].substring(2)+pad(64-message[0].substring(2).length) + web3.utils.sha3('withdraw(uint256,bytes32[],uint256[],uint256[],uint8[],bytes1[],bytes32[],bytes32[])').substring(0,10);
            console.log(_whole_msg)            
            var _vsignature = web3.eth.accounts.sign(_whole_msg, verify_private_key).v.toString();
            var _rsignature = web3.eth.accounts.sign(_whole_msg, verify_private_key).r.toString();
            var _ssignature = web3.eth.accounts.sign(_whole_msg, verify_private_key).s.toString();
        ...
        case 3:
            ...
            var _whole_msg = receiver +type.toString() +access_time.toString() + _index.toString()+ message[0].substring(2)+pad(64-message[0].substring(2).length) + web3.utils.sha3('withdraw(uint256,bytes32[],uint256[],uint256[],uint8[],bytes1[],bytes32[],bytes32[])').substring(0,10)+ argValue;
            console.log(_whole_msg) 
            var _vsignature = web3.eth.accounts.sign(_whole_msg, verify_private_key).v.toString();
            var _rsignature = web3.eth.accounts.sign(_whole_msg, verify_private_key).r.toString();
            var _ssignature = web3.eth.accounts.sign(_whole_msg, verify_private_key).s.toString();
...
function verifyPolicy(type, msg, contract_name){
    var counter = 0;
    if (counter == 0){
        white_list = JSON.parse(policyStorage.getItem('Banker')).white;
        black_list = JSON.parse(policyStorage.getItem('Banker')).black;
        fun_white_list = JSON.parse(policyStorage.getItem(contract_name)).function.withdraw.white;
        fun_black_list = JSON.parse(policyStorage.getItem(contract_name)).function.withdraw.black;
        var_white_list = JSON.parse(policyStorage.getItem(contract_name)).args.amount.white;
        var_black_list = JSON.parse(policyStorage.getItem(contract_name)).args.amount.black;
        counter = counter+1;
    }
    switch(type){
        case 1:
...
function addPolicy(contract_name, method_name, white_list, black_list, tools){
    var _white_list = [];
    for (var i=0; i< white_list.length; i++){
    ...
    _list['tools'] = 'ECFChecker';
    var _function = {};
    _function['withdraw'] = _list;
    var _args = {};
    _args['amount'] = _list;
